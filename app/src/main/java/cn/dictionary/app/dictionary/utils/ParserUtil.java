...
import cn.dictionary.app.dictionary.entity.Web;
...
public class ParserUtil {
    ...
    public static Words ParseJSONForWord(String jsonData) {
    ...
    public static DailySentence ParseJSONForDailyEnglish(String jsonData) {
        JSONObject jo;
        ...
        try {
            ...
            //音频
            if (jo.has("tts")) {
                dailySentence.setVoicePath(jo.getString("tts"));
            }
            //英文内容
            if (jo.has("content")) {
                dailySentence.setEnglishContent(jo.getString("content"));
            }
            //中文内容
            if (jo.has("note")) {
                dailySentence.setChineseContent(jo.getString("note"));
            }
            //图片
            if (jo.has("picture2")) {
                dailySentence.setPicturePath(jo.getString("picture2"));
            }
            //时间
            if (jo.has("dateline")) {
                dailySentence.setDateline(jo.getString("dateline"));
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
